...
export function ReportManager() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const response = await axios.get<ReportWithId[]>("/api/reports");
  ...
  const addDoneItem = () =>
    setDoneItems([...doneItems, { description: "", startTime: "", endTime: "" }]);
  ...
  const handleSave = async () => {
    ...
    const reportData = {
      ...
      status,
      doneItems:
        status === "Complete" ? doneItems.filter((item) => item.description.trim()) : [],
      todoItems:
        status === "Incomplete"
  ...
  const generateReportText = (
    ...
    const itemsToProcess =
      (report.doneItems as DoneItemUI[] | undefined) ||
      (status === "Complete" ? doneItems : []);
    const todoItemsToProcess =
      report.todoItems || (status === "Incomplete" ? todoItems : []);
    ...
    if (report.status === "Complete" && itemsToProcess.length > 0) {
      ...
      itemsToProcess.forEach((item: DoneItemUI) => {
        if (item.description.trim()) {
          ...
          const doc = item.documentationUrl ? ` [doc](${item.documentationUrl})` : "";
          output += `[done] ${item.description} ${timeRange}${doc}\n`;
        }
      });
    }
  ...
  const handleEdit = (report: ReportWithId) => {
    ...
    setTodoItems(
      report.todoItems.length > 0 ? (report.todoItems as ITodoItem[]) : [{ description: "" }]
    );
  };
              ...
              {doneItems.map((item: DoneItemUI, index: number) => (
                ...
                >
                  <Input
                    ...
                    onChange={(e) => handleDoneChange(index, "description", e.target.value)}
                  ...
                  <div className="flex items-center gap-2 justify-end">
                    <Input
                      ...
                      onChange={(e) => handleDoneChange(index, "startTime", e.target.value)}
